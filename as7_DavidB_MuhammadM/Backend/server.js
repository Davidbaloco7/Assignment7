...
async function saveData(req,res){
    ...
    let an =  await db.run('UPDATE Games SET  game_name = ?, publisher = ?, developer = ? , score = ? WHERE id = ?',
        [val.game_name, val.publisher,val.developer,val.score, id]
    );
    db.close;
    if(an.changes > 0){
...
async function saveAllData(req,res){
    ...
    for(let key in val){
        ...
        [content.game_name, content.publisher,content.developer,content.score]
     );
     console.log(an);
    }
    db.close;
...
async function addData(req,res){
    ...
    let an =  await db.run('INSERT INTO Games (game_name, publisher,developer,score) VALUES (?,?,?,?)',
        [val.game_name, val.publisher,val.developer,val.score]
     );
    db.close;
